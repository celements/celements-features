...
public class PresentationMetadataContractTest extends AbstractComponentTest {
    ...
    public void prepareTest() throws Exception {
        ...
        modelUtils = registerComponentMock(ModelUtils.class);
        layoutService = registerComponentMock(LayoutServiceRole.class);
        pageTypeResolver = registerComponentMock(IPageTypeResolverRole.class);
        IWebUtilsService webUtils = registerComponentMock(IWebUtilsService.class);
        ...
        expect(webUtils.getRefLocalSerializer()).andReturn(Utils.getComponent(EntityReferenceSerializer.class, "local"))
                .anyTimes();
    ...
    @Test
    public void map_usesRealNavigationForExactRootMetadata() throws Exception {
        XWikiContext xwikiContext = getXContext();
        ...
        PageTypeReference firstPageType = createDefaultMock(PageTypeReference.class);
        PageTypeReference secondPageType = createDefaultMock(PageTypeReference.class);
        ...
        INavigation navigation1 = mapper.createNavigation(definition, 1);
        INavigation navigation2 = mapper.createNavigation(definition, 2);
        List<SlideResponse> response1 = mapper.map(definition, slides, navigation1);
        List<SlideResponse> response2 = mapper.map(definition, slides, navigation2);
        ...
        assertEquals(
                List.of("cel_cm_presentation_treenode", "first", "cel_nav_odd", "cel_nav_item1", "cel_nav_isLeaf",
                        "cel_nav_nodeSpace_Content", "cel_nav_nodeName_First", "FirstPageType", "layout_About-Layout"),
                response1.get(0).cssClasses());
        assertEquals(List.of("cel_cm_presentation_treenode", "last", "cel_nav_even", "cel_nav_item2",
        ...
        assertEquals(response1.get(0).cssClasses(), response2.get(0).cssClasses());
        assertEquals(response1.get(1).cssClasses(), response2.get(1).cssClasses());
    }
