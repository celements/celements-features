...
public class MailSenderApiTest {
   ...
   *
   * // The plugin init creates a XWiki.Mail document if it doesn't exist and ensure it has the
   * // correct
   * // class properties.
   ...
   * this.mockXWiki.stubs().method("getXWikiPreferenceAsInt").withAnyArguments()
   * .will(returnValue(25));
   * this.mockXWiki.stubs().method("getXWikiPreference").with(eq("smtp_from"), ANYTHING)
   * .will(returnValue(""));
   * this.mockXWiki.stubs().method("getXWikiPreference").with(eq("smtp_server_username"), ANYTHING)
   * .will(
   * returnValue(""));
   * this.mockXWiki.stubs().method("getXWikiPreference").with(eq("smtp_server_password"), ANYTHING)
   * .will(
   * returnValue(""));
   * this.mockXWiki.stubs().method("getXWikiPreference").with(eq("javamail_extra_props"), ANYTHING)
   * .will(
   * returnValue(""));
   *
   ...
   * mail.setTextPart("Text content");
   *
   * MailConfiguration config = this.api
   * .createMailConfiguration(new com.xpn.xwiki.api.XWiki(this.xwiki, getContext()));
