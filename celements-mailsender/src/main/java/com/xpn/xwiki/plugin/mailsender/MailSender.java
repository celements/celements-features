...
public interface MailSender {
   ...
   * 
   * @param xwiki
   *          the XWiki object used to get the default values from the XWiki Preferences
   *          ("smtp_server" and
   *          "smtp_from").
   * @return A mail server configuration, initialized with values from XWiki Preferences, but which
   *         can be overriden
   *         by users.
   */
  ...
  /**
   * Generic method for sending emails. The passed Mail object has to be populated by the caller to
   * set the correct
   * fields. All the other <code>sendHtmlXXX()</code> and <code>sendTextXXX()</code> methods are
   * specialized helper
   * versions of this generic method.
   * 
   * @param mail
   *          the already populated mail Object to be sent
   * @return 0 on success, -1 on failure. On failure the error message is stored in the XWiki
   *         context under the
   *         "error" key.
   */
   ...
   * Generic method for sending emails. The passed Mail object has to be populated by the caller to
   * set the correct
   * fields. The passed Mail Configuration allows the user to override the default connection
   * properties (SMTP host,
   * SMTP port, SMTP from, etc). All the other <code>sendHtmlXXX()</code> and
   * <code>sendTextXXX()</code> methods are
   ...
   * @param alternative
   *          the alternative text offered to the mail client
   * @param attachments
   ...
   *         "error" key.
   */
  int sendHtmlMessage(String from, String to, String cc, String bcc, String subject, String body,
      String alternative,
  ...
  /**
   * Sends a raw message. The message can contain additional headers at the start, which are parsed
   * and correctly sent
   * as additional headers (Bcc, Subject, Reply-To, etc.). The actual message is treated as plain
   * text.
   * 
   * @param from
   *          the sender
   ...
   *          the receiver
   * @param rawMessage
   *          the raw message, containing additional headers and the actual message
   * @return 0 on success, -1 on failure. On failure the error message is stored in the XWiki
   *         context under the
   *         "error" key.
   */
