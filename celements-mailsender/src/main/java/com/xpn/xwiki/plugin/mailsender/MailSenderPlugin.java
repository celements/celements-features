...
public class MailSenderPlugin extends XWikiDefaultPlugin {
  ...
  /**
   * Since Java uses full Unicode Strings and email clients manage it we force email encoding to
   * UTF-8. XWiki encoding
   * must be used when working with storage or the container since they can be configured to use
   * another encoding,
   ...
   * {@link #sendMailFromTemplate(String, String, String, String, String, String, VelocityContext, XWikiContext)}
   * was
   * not found.
   */
  ...
  public Multipart createMimeMultipart(Mail mail, XWikiContext context)
    ...
    if (mail.getHtmlPart() == null && mail.getAttachments() != null) {
      ...
      // Create the HTML part of the email, define the html as a multipart/related in case there are
      // images
      ...
      // Loop over the attachments of the email, add images used from the HTML to the list of
      // attachments to be
      // embedded with the HTML part, add the other attachements to the list of attachments to be
      // attached to the
      // email.
      for (Attachment attachment : rawAttachments) {
      ...
      // Add the images to the HTML multipart (they should be hidden from the mail reader attachment
      // list)
  ...
  /**
   * Splits a raw mail into headers and the actual content, filling in a {@link Mail} object. This
   * method should be
   * compliant with RFC 2822 as much as possible. If the message accidentally starts with what looks
   * like a mail
   * header, then that line <strong>WILL</strong> be considered a header; no check on the semantics
   * of the header is
   * performed.
   * 
   * @param rawMessage
   *          the raw content of the message that should be parsed
   * @param toMail
   *          the {@code Mail} to create
   ...
   *          Mail bcc
   * @param vcontext
   *          The Velocity context to prepare
   ...
   * @return True in any case (TODO ?)
   */
  public boolean sendMails(Collection<Mail> emails, MailConfiguration mailConfiguration,
   ...
   *          Email Carbon Copy
   * @param bcc
   *          Email Hidden Carbon Copy
   * @param language
   *          Language of the email
   * @param vcontext
   *          Velocity context passed to the velocity renderer
   * @return True if the email has been sent
   */
  public int sendMailFromTemplate(String templateDocFullName, String from, String to, String cc,
      String bcc,
    ...
    try {
      ...
      VelocityContext updatedVelocityContext = prepareVelocityContext(from, to, cc, bcc, vcontext,
          context);
      XWiki xwiki = context.getWiki();
      ...
      String msg = XWikiVelocityRenderer.evaluate(txtContent, templateDocFullName,
          updatedVelocityContext, context);
      String html = XWikiVelocityRenderer.evaluate(htmlContent, templateDocFullName,
          updatedVelocityContext, context);
